...
class PIIDetectionEntity(BaseModel):
    ...
    location: Optional[str] = Field(None, description="Location description (for images)")


class PIIDetectionResponse(BaseModel):
    """Response model for PII detection."""
    entities: List[PIIDetectionEntity] = Field(default_factory=list, description="List of detected PII entities")
...
class PydanticAIEngine:
    ...
    def __init__(self, config: Config):
        ...
        self.config = config
        self.enabled = (
            getattr(config, 'use_ollama', False) or
            getattr(config, 'use_openai_compatible', False) or
            getattr(config, 'use_multimodal', False) or
    ...
    def _get_api_key(self) -> Optional[str]:
        ...
        if self.provider == 'ollama':
            ...
            return (
                getattr(self.config, 'multimodal_api_key', None) or
    ...
    def _get_base_url(self) -> Optional[str]:
        ...
        if self.provider == 'ollama':
            ...
            return (
                getattr(self.config, 'multimodal_api_base', None) or
                getattr(self.config, 'openai_api_base', 'https://api.openai.com/v1')
            )
        elif self.provider == 'openai':
    ...
    def detect(self, text: str, labels: list[str] | None = None, 
        ...
        if image_path or (getattr(self.config, 'use_multimodal', False) and text and os.path.exists(text)):
    ...
    def _detect_image(self, image_path: str, labels: Optional[List[str]]) -> List[DetectionResult]:
...
Extract all PII entities and return them in the structured format."""
    ...
    def _convert_results(self, response: PIIDetectionResponse, 
                        image_path: Optional[str] = None) -> List[DetectionResult]:
