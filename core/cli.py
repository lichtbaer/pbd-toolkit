...
def scan(
    # Required arguments
    path: str = typer.Argument(..., help="Root directory under which to recursively search for PII"),
    
    # Analysis methods (at least one required, validated later)
    regex: bool = typer.Option(False, "--regex", help="Use regular expressions for analysis"),
    ner: bool = typer.Option(False, "--ner", help="Use AI-based Named Entity Recognition for analysis (GLiNER)"),
    
    # Additional NER engines
    spacy_ner: bool = typer.Option(False, "--spacy-ner", help="Use spaCy NER models for detection"),
    spacy_model: str = typer.Option("de_core_news_lg", "--spacy-model", 
    ...
    openai_compatible: bool = typer.Option(False, "--openai-compatible",
                                           help="Use OpenAI-compatible API for detection"),
    openai_api_base: str = typer.Option("https://api.openai.com/v1", "--openai-api-base",
                                        help="OpenAI-compatible API base URL"),
    openai_api_key: Optional[str] = typer.Option(None, "--openai-api-key",
                                                  help="OpenAI API key (or set OPENAI_API_KEY env var)"),
    openai_model: str = typer.Option("gpt-3.5-turbo", "--openai-model",
                                     help="OpenAI model to use (default: gpt-3.5-turbo)"),
    
    # Multimodal image detection
    multimodal: bool = typer.Option(False, "--multimodal",
                                   help="Use multimodal models for image PII detection"),
    multimodal_api_base: Optional[str] = typer.Option(None, "--multimodal-api-base",
                                                      help="Multimodal API base URL (defaults to --openai-api-base)"),
    multimodal_api_key: Optional[str] = typer.Option(None, "--multimodal-api-key",
                                                     help="Multimodal API key (defaults to --openai-api-key or OPENAI_API_KEY)"),
    multimodal_model: str = typer.Option("gpt-4-vision-preview", "--multimodal-model",
                                         help="Multimodal model to use (default: gpt-4-vision-preview)"),
    multimodal_timeout: int = typer.Option(60, "--multimodal-timeout",
                                           help="Multimodal API timeout in seconds (default: 60)"),
    
    # PydanticAI unified engine
    pydantic_ai: bool = typer.Option(False, "--pydantic-ai",
                                     help="Use PydanticAI unified LLM engine (recommended, replaces --ollama, --openai-compatible, --multimodal)"),
    pydantic_ai_provider: str = typer.Option("openai", "--pydantic-ai-provider",
                                             help="LLM provider for PydanticAI (default: openai)",
                                             case_sensitive=False),
    pydantic_ai_model: Optional[str] = typer.Option(None, "--pydantic-ai-model",
                                                     help="Model name for PydanticAI (auto-determined from provider if not specified)"),
    pydantic_ai_api_key: Optional[str] = typer.Option(None, "--pydantic-ai-api-key",
                                                        help="API key for PydanticAI (or use provider-specific env vars)"),
    pydantic_ai_base_url: Optional[str] = typer.Option(None, "--pydantic-ai-base-url",
                                                        help="Base URL for PydanticAI (for custom endpoints)"),
    
    # File type detection
    use_magic_detection: bool = typer.Option(False, "--use-magic-detection",
                                             help="Enable file type detection using magic numbers (file headers)"),
    magic_fallback: bool = typer.Option(True, "--magic-fallback/--no-magic-fallback",
                                        help="Use magic detection as fallback when extension doesn't match (default: True)"),
    
    # Optional arguments
    outname: Optional[str] = typer.Option(None, "--outname",
                                          help="Optional parameter; string which to include in the file name of all output files"),
    whitelist: Optional[str] = typer.Option(None, "--whitelist",
    ...
    stop_count: Optional[int] = typer.Option(None, "--stop-count",
                                              help="Optional parameter; stop analysis after N files"),
    ...
    format: str = typer.Option("csv", "--format",
                               ...
                               case_sensitive=False),
    summary_format: str = typer.Option("human", "--summary-format",
    ...
    output_dir = args.output_dir if hasattr(args, 'output_dir') and args.output_dir else constants.OUTPUT_DIR
    ...
    if not any(enabled_methods):
        typer.echo(
            translate_func("At least one detection method must be enabled (--regex, --ner, --spacy-ner, --ollama, --openai-compatible, --multimodal, or --pydantic-ai)."),
            err=True
        )
        raise typer.Exit(code=constants.EXIT_INVALID_ARGUMENTS)
    ...
    if config_obj.use_ner and config_obj.ner_model is None:
        typer.echo(
            translate_func("GLiNER NER is enabled but model could not be loaded. Please check the error messages above."),
            err=True
        )
        raise typer.Exit(code=constants.EXIT_CONFIGURATION_ERROR)
    ...
    context.logger.info(context._("Performance of analysis: {} analyzed files per second").format(
    ...
    if context.config.use_ner and context.statistics.ner_stats.total_chunks_processed > 0:
        ...
        context.logger.info(context._("Total NER processing time: {:.2f}s").format(
            context.statistics.ner_stats.total_processing_time))
        context.logger.info(context._("Average time per chunk: {:.3f}s").format(
    ...
    if not (hasattr(args, 'quiet') and args.quiet):
        summary_format = getattr(args, 'summary_format', 'human') if hasattr(args, 'summary_format') else 'human'
        ...
        if summary_format == 'json':
            ...
            if context.statistics.start_time:
                typer.echo(f"{context._('Started:')}     {context.statistics.start_time.strftime('%Y-%m-%d %H:%M:%S')}")
            if context.statistics.end_time:
                typer.echo(f"{context._('Finished:')}    {context.statistics.end_time.strftime('%Y-%m-%d %H:%M:%S')}")
            typer.echo(f"{context._('Duration:')}    {context.statistics.duration}")
            typer.echo()
            typer.echo(context._("Statistics:"))
            typer.echo(f"  {context._('Files scanned:')}      {context.statistics.total_files_found:,}")
            typer.echo(f"  {context._('Files analyzed:')}     {context.statistics.files_processed:,}")
            typer.echo(f"  {context._('Matches found:')}      {context.statistics.matches_found:,}")
            typer.echo(f"  {context._('Errors:')}             {context.statistics.total_errors:,}")
            typer.echo()
            typer.echo(context._("Performance:"))
            typer.echo(f"  {context._('Throughput:')}         {context.statistics.files_per_second} {context._('files/sec')}")
            typer.echo()
            if errors:
