...
EUPL_COMPATIBLE_LICENSES = [
    ...
    "OSL-2.1", "OSL-3.0",
    "EPL-1.0", "EPL-2.0",  # EPL-2.0 is also generally compatible
...
PERMISSIVE_LICENSES = [
    "MIT", "BSD", "Apache-2.0", "ISC", "Python-2.0", "ZPL-2.0", "ZPL-2.1",
    "PSF", "Public-Domain", "Unlicense", "CC0-1.0"
]
...
def get_package_info(package_name: str) -> Optional[Dict]:
    ...
    try:
        ...
        with urllib.request.urlopen(url, timeout=10) as response:
            ...
            if (not info.get("license") or info.get("license") == "UNKNOWN") and info.get("classifiers"):
                    ...
                    if classifier.startswith("License ::"):
                        ...
                        if len(license_parts) >= 2:
                            ...
                            if "Apache Software License" in license_from_classifier:
                                ...
                                break
                            elif "GNU General Public License (GPL)" in license_from_classifier:
                                # Try to determine version from description or default to GPL-3.0
                                desc = info.get("description", "").upper()
                                if "GPL V3" in desc or "GPL-3" in desc or "GPLV3" in desc:
                                    info["license"] = "GPL-3.0"
                                elif "GPL V2" in desc or "GPL-2" in desc or "GPLV2" in desc:
                                    info["license"] = "GPL-2.0"
                                else:
                                    info["license"] = "GPL-3.0"  # Default to GPL-3.0
                                break
                            elif "GNU Library or Lesser General Public License (LGPL)" in license_from_classifier:
        ...
        print(f"  Warning: Could not fetch info for {package_name}: {e}", file=sys.stderr)
        return None
...
def normalize_license(license_str: str) -> List[str]:
    ...
    for sep in [",", " or ", " OR ", " and ", " AND "]:
        if sep in license_str:
            licenses.extend([l.strip() for l in license_str.split(sep)])
            break
...
def main():
    ...
    print(f"Detailed report saved to license_report.json")
    print()
