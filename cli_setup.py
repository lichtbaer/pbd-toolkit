...
def __setup_args(translate_func: Callable[[str], str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=translate_func(
            "Use AI-based Named Entity Recognition for analysis (GLiNER)"
        ),
    )
    ...
    parser.add_argument(
        "--multimodal-api-key",
        help=translate_func(
            "Multimodal API key (defaults to --openai-api-key or OPENAI_API_KEY)"
        ),
    )
    parser.add_argument(
        "--multimodal-model",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=translate_func(
            "Use PydanticAI unified LLM engine (recommended, replaces --ollama, --openai-compatible, --multimodal)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pydantic-ai-model",
        help=translate_func(
            "Model name for PydanticAI (auto-determined from provider if not specified)"
        ),
    )
    parser.add_argument(
        "--pydantic-ai-api-key",
        help=translate_func(
            "API key for PydanticAI (or use provider-specific env vars)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=translate_func(
            "Enable file type detection using magic numbers (file headers)"
        ),
    )
    parser.add_argument(
        "--magic-fallback",
        ...
        default=True,
        help=translate_func(
            "Use magic detection as fallback when extension doesn't match (default: True)"
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store",
        help=translate_func(
            "Optional parameter; string which to include in the file name of all output files"
        ),
    )
    parser.add_argument(
        "--whitelist",
        action="store",
        help=translate_func(
            "Optional parameter; relative path to a text file containing one string per line. These strings will be matched against potential findings to exclude them from the output."
        ),
    )
    parser.add_argument(
        "--stop-count",
    ...
    parser.add_argument(
        ...
        default="human",
        help=translate_func(
            "Format for summary output (default: human). Use 'json' for machine-readable output."
        ),
    )
    parser.add_argument(
        "--no-header",
