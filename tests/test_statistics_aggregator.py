...
def create_test_match(text: str, file: str, type: str, engine: str = "regex", score: float = None) -> PiiMatch:
...
def test_aggregator_add_match():
    ...
    match1 = create_test_match("test@example.com", "/path/file1.pdf", "REGEX_EMAIL", "regex")
    match2 = create_test_match("John Doe", "/path/file2.docx", "NER_PERSON", "gliner", 0.95)
    ...
    assert stats["statistics_by_dimension"]["contact_information"]["by_type"]["REGEX_EMAIL"] == 1
...
def test_aggregator_file_tracking():
    ...
    match1 = create_test_match("test@example.com", "/path/file1.pdf", "REGEX_EMAIL", "regex")
    match2 = create_test_match("test2@example.com", "/path/file1.pdf", "REGEX_EMAIL", "regex")
    ...
    assert stats["statistics_by_dimension"]["contact_information"]["files_affected"] == 1
...
def test_aggregator_summary():
    ...
    agg.add_match(create_test_match("test@example.com", "/path/file1.pdf", "REGEX_EMAIL", "regex"))
    agg.add_match(create_test_match("John Doe", "/path/file2.docx", "NER_PERSON", "gliner"))
    agg.add_match(create_test_match("123456789", "/path/file3.pdf", "REGEX_IBAN", "regex"))
    agg.add_match(create_test_match("Health data", "/path/file4.txt", "NER_HEALTH", "gliner", 0.88))
    ...
    assert summary["dimensions_detected"] == 3  # contact_information, identity, financial, health
...
def test_aggregator_confidence_distribution():
    ...
    agg.add_match(create_test_match("Match1", "/path/file1.pdf", "NER_PERSON", "gliner", 0.3))
    agg.add_match(create_test_match("Match2", "/path/file1.pdf", "NER_PERSON", "gliner", 0.6))
    agg.add_match(create_test_match("Match3", "/path/file1.pdf", "NER_PERSON", "gliner", 0.8))
    agg.add_match(create_test_match("Match4", "/path/file1.pdf", "NER_PERSON", "gliner", 0.95))
